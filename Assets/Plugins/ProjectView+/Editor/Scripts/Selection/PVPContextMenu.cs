...
namespace ProjectViewPlus
{
    public static class PVPContextMenu
    {
        ...
        private static GenericMenu contextMenu
        {
            get
            {
                if (menu == null)
                {
                    ...
                    menu.AddItem(new GUIContent("Create/AnimationClip"), false, CreateAnimationClip);
        ...
        private static void CreateFolder()
        {
            ...
            // Check if the folder name already exists in the project
            int folderCount = 0;
            string folderName = "NewFolder";
            while (AssetDatabase.IsValidFolder(path))
            {
                folderCount++;
                folderName = "NewFolder" + (folderCount > 0 ? folderCount.ToString() : "");
            ...
            var newFolder = new PVPFolder(path, contextElement.SelectableContextFolder, contextElement.SelectableContextFolder.Depth + 1);
            contextElement.SelectableContextFolder.AddChildFolder(newFolder);
        }
